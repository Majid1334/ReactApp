...
let diagramInstance: Diagram | null = null;
...
function MindMap() {
  ...
  function getTool(action: string): ToolBase {
    if (action === "leftHandle") {
      return new LeftExtendTool(diagramInstance!.commandHandler);
    } else if (action === "rightHandle") {
      return new RightExtendTool(diagramInstance!.commandHandler);
    } else if (action === "delete") {
      return new DeleteClick(diagramInstance!.commandHandler);
    }
    // fallback to a default ToolBase instance to ensure a ToolBase is always returned
    return new ToolBase(diagramInstance!.commandHandler);
  }
  ...
  function getTextEditValue(selectObject: any, node: any) {
    ...
    diagramInstance!.select([(diagramInstance!.nameTable as any)[newNode.id]]);
    const selectedNodes = diagramInstance!.selectedItems?.nodes;
    if (selectedNodes && selectedNodes.length > 0) {
      diagramInstance!.startTextEdit(selectedNodes[0]);
    }
  }
  ...
  return (
    <div className="control-pane">
      <div className="control-section">
        <div style={{ width: "100%" }}>
          <DiagramComponent
            ref={(element: DiagramComponent | null) => {
              diagramInstance = element as unknown as Diagram | null;
            }}
            id="diagram"
              ...
              if (arg.state === "Changing") {
                if (arg.newValue[0] instanceof Node) {
                  if (
                    ...
                    const handles = diagramInstance.selectedItems.userHandles;
